...
def llm_pipeline(filepath):
    summarizer = pipeline('summarization', model=base_model, tokenizer=tokenizer, max_length=2000, min_length=500)
    input_text = file_preprocessing(filepath)
    ...
    try:
        ...
        summary = result[0]['summary_text'].replace(". ", ".\n")
    except Exception as e:
...
def main():
    ...
    if uploaded_file is not None:
        if st.button("Summarize"):
            ...
            with col1:
                st.info("Uploaded PDF File")
                st.pdf(temp_file_path)  # Streamlit's built-in PDF Viewer
            ...
            with col2:
                ...
                if target_language_code != "en":
                    try:
                        ...
                        st.write(f"Translated Summary ({selected_language}):", translated_summary)
                    except ValueError as e:
                        st.error(f"Error translating summary: {e}")
                        return
                else:
                ...
                # Display Summary
                st.text_area("Summary", translated_summary, height=300)
                ...
                export_to_word(translated_summary, word_filename)
                with open(word_filename, "rb") as word_file:
                    st.download_button(
                        label="Download as Word (.docx)",
                        data=word_file,
                ...
                export_to_pdf(translated_summary, pdf_filename)
                with open(pdf_filename, "rb") as pdf_file:
                    st.download_button(
                        label="Download as PDF (.pdf)",
                        data=pdf_file,
